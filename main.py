...
# Body - 미디어 타입을 명시적으로 지정해주기 위한 import
...
# 로그
import logging
print("--- main.py 파일 로드 시작 ---") # 이후 주석 처리

# config.py에서 설정 가져오기 - 내 파일
from config import settings, create_initial_directories

# API Key Value Error를 위한 디버깅
print(f"--- settings.GOOGLE_API_KEY in main.py: {'LOADED' if settings.GOOGLE_API_KEY else 'NOT LOADED'} ---")
...
async def search_rag_system(question_text: str = Body(..., media_type="text/plain")):
    logger.info(f"'/search' 엔드포인트 수신 질문: {question_text}")
